...
CTxDestination DecodeDestination3(const std::string& str, const int pubkey_prefix, const int script_prefix)
{
    std::vector<unsigned char> data;
    uint160 hash;
    if (DecodeBase58Check(str, data)) {
        ...
        // const std::vector<unsigned char>& pubkey_prefix = params.Base58Prefix(CChainParams::PUBKEY_ADDRESS);
        const std::vector<unsigned char>& pubkey_prefix_v = std::vector<unsigned char>(1,pubkey_prefix);
        ...
        // The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
        // const std::vector<unsigned char>& script_prefix = params.Base58Prefix(CChainParams::SCRIPT_ADDRESS);
        const std::vector<unsigned char>& script_prefix_v = std::vector<unsigned char>(1,script_prefix);
...
CTxDestination DecodeDestination2(const std::string& str, const int pubkey_prefix, const int script_prefix)
{
