...
void komodo_args(char *argv0)
{
	...
	ASSETCHAINS_PAYTXTOADDY = GetArg("-ac_paytxtoaddy",0);
...
fprintf(stderr,"extralen.%d before disable bits\n",extralen);
	    ...
	    if (ASSETCHAINS_ACTIVEUSERREWARDS[0] == 1)
	    {
                for ( int i = 0; i <= 5; i++ )
		{
		    extralen += iguana_rwnum(1,&extraptr[extralen],sizeof(ASSETCHAINS_ACTIVEUSERREWARDS[i]),(void *)&ASSETCHAINS_ACTIVEUSERREWARDS[i]);
		}					
	    }
        }
