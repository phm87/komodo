            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                    ...
                    (!coinControl || !coinControl->HasSelected() || coinControl->IsSelected((*it).first, i)))
                {
                    if ( KOMODO_EXCHANGEWALLET == 0 )
                    {
                        ...
                        if ( ( ASSETCHAINS_SYMBOL[0] == 0 || ASSETCHAINS_ACTIVEUSERREWARD == 1 ) && chainActive.LastTip() != 0 && chainActive.LastTip()->GetHeight() >= 60 )
                        {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
                {
                    ...
                    if ( KOMODO_EXCHANGEWALLET == 0 && ( ASSETCHAINS_SYMBOL[0] == 0 || ASSETCHAINS_ACTIVEUSERREWARD == 1 ) )
                    {
                ...
                //}
                if ( ( ASSETCHAINS_SYMBOL[0] == 0 || ASSETCHAINS_ACTIVEUSERREWARD == 1 ) && DONATION_PUBKEY.size() == 66 && interest2 > 5000 )
                {
